...
from utils.traj_distance import calculate_distance

from ..config import Config
from ..model.lambdaloss import lambdaLoss
from ..model.tsmini import TSMini
from ..utils import tool_funcs
from ..utils.cellspace import create_cellspace
from ..utils.data_loader import (
    TrajSimiDatasetTraining,
    read_trajsimi_traj_dataset,
)
from ..utils.traj import padding_traj, preprocess_traj
...
def _build_simi_dataset(trains, evals, tests, metric, n_jobs=None):
    logging.info("[trajsimi simi dataset] Start building.")
    _time = time.time()

    trains_simi = _pairwise_distance_matrix(trains, metric, n_jobs=n_jobs)
    evals_simi = _pairwise_distance_matrix(evals, metric, n_jobs=n_jobs)
    tests_simi = _pairwise_distance_matrix(tests, metric, n_jobs=n_jobs)

    max_distance = float(
        max(np.max(trains_simi), np.max(evals_simi), np.max(tests_simi), 1.0)
    )

    logging.info(
        "[trajsimi simi dataset] Built. @={:.2f}, trains/evals/tests={}/{}/{}".format(
            time.time() - _time,
            len(trains_simi),
            len(evals_simi),
            len(tests_simi),
...
class TrajSimi:
    def __init__(self):
        ...
        self.space = create_cellspace(
            *self.dic_datasets["trajs_space_range"],
            Config.cell_size,
            Config.cell_size,
        ...
        self.dataset_simi_trains = torch.tensor(
            self.dataset_simi_trains, device=Config.device, dtype=torch.float
        )
        self.dataset_simi_trains = (
            self.dataset_simi_trains + self.dataset_simi_trains.T
    ...
    def train(self):
        ...
        for i_ep in range(Config.trajsimi_epoch):
            ...
            for i_batch, batch in enumerate(self.train_dataloader):
                ...
                if i_batch % 1000 == 0:
                    logging.info(
                        "training. ep-batch={}-{}, loss={:.4f}, @={:.3f}, gpu={}, ram={}".format(
                            i_ep,
                            i_batch,
                            loss.item(),
                            time.time() - _time_ep,
            ...
            logging.info(
                "training. i_ep={}, loss={:.4f}/{:.4f}/{:.4f}, @={:.3f}".format(
            ...
            logging.info(
                "eval.     i_ep={}, mseloss={:.4f}, hr={:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f}, gpu={}, ram={}, @={:.3f}".format(
            ...
            if bad_counter == bad_patience or i_ep + 1 == Config.trajsimi_epoch:
                ...
                logging.info(
                    "training end. @={:.3f}, best_epoch={}, best_hr_eval={:.4f}".format(
                        training_endtime - training_starttime,
                        best_epoch,
                        best_hr_eval,
                    )
                )
        ...
        logging.info(
            "test. @={:.3f}, mseloss={:.4f}, hr={:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f}, gpu={}, ram={}, @={:.3f}".format(
        ...
        return {
            ...
            "hr10": test_metrics[1],
            "hr50": test_metrics[2],
            "hr50in10": test_metrics[3],
            "hr5": test_metrics[4],
            "hr20": test_metrics[5],
            "hr20in5": test_metrics[6],
        }
    ...
    def load_trajsimi_dataset():
        ...
        return {
            ...
            "trajs_space_range": trajs_merc_range,
        }
