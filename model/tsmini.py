...
from .embeder import ConvEmbeder
from .msa_llm import LlamaModel as MSALLM
from .msa_llm import ModelArgs
...
class TSMini(nn.Module):
    ...
    def forward(self, trajs, trajs_len):
        ...
        src_padding_mask = (
            torch.arange(patch_len_max, device=device)[None, :] >= patch_len[:, None]
        )

        kwargs = {
            "src": patch_embs.permute(1, 0, 2),
            "attn_mask": None,
